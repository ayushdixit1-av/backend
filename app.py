...
# -------------------------
# Helper to build candidate public URLs for signature validation
# -------------------------
def _build_public_url_candidates(req):
    """
    Build a list of URL candidates Twilio might have signed.
    Uses forwarded headers when available and also produces common variants.
    """
    candidates = []

    # Use X-Forwarded-Proto and X-Forwarded-Host if present (Railway / proxies)
    ...
    # Also include variants (https/http) with forwarded host or Host
    host2 = req.headers.get("Host", None)
    if host2:
        candidates.append(f"https://{host2}{path}")
        candidates.append(f"http://{host2}{path}")

    # Try request.url which may be internal (http://127.0.0.1:PORT/...)
    candidates.append(req.url)

    # If request.url has http, try https variant
    try:
...
# -------------------------
# Twilio request validation decorator (tries multiple URL candidates)
# -------------------------
def require_twilio(f):
    ...
    def wrapper(*args, **kwargs):
        if validator:
            ...
            app.logger.debug("Twilio signature header: %s", sig)
            app.logger.debug("Trying URL candidates for validation: %s", url_candidates)
            valid = False
            for candidate in url_candidates:
                try:
                    if validator.validate(candidate, params, sig):
                        valid = True
                        app.logger.debug("Twilio signature validated with URL: %s", candidate)
                        break
                except Exception as ex:
                    app.logger.debug("Validator exception for URL %s: %s", candidate, ex)
                    continue
            if not valid:
                # Extra debug: show headers and params in logs (be careful in prod)
                app.logger.warning("Invalid Twilio signature. Tried URLs: %s", url_candidates)
                app.logger.debug("Form params: %s", params)
